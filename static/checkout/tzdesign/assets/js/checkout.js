(function () {
    ...
    var WEB3 = '/checkout/tzdesign/assets/web3icons';
    var iconCache = {};
    ...
    function initI18n() {
        ...
        return new Promise(function (resolve) {
            ...
            i18next.init({ lng: lang, debug: false, resources: {} }, function (err) {
                ...
                fetch('/checkout/tzdesign/assets/locales/' + lang + '.json')
    ...
    function switchLang(l) {
        ...
        lang = l;
        fetch('/checkout/tzdesign/assets/locales/' + l + '.json')
            .then(function (r) { return r.json(); })
            .then(function (d) {
        ...
        function render(filter) {
            ...
            arr.forEach(function (it) {
                ...
                el.addEventListener('click', function (e) {
                    ...
                    if (img) {
                        img.classList.remove('select-img-placeholder');
                        img.style.backgroundImage = 'url("' + it.iconSrc + '")';
                        img.setAttribute('aria-label', it.label);
                    }
    ...
    function resetTrigger(id, ph) {
        ...
        var img = trigger.querySelector('.select-img');
        if (img) {
            img.classList.add('select-img-placeholder');
    ...
    function specialPaymentTypeFromMethod(method) {
        if (!method) return '';
        if (method.network === 'alipay') return 'alipay';
        if (method.network === 'duolabao') return 'duolabao';
        if (method.network === 'stripe') {
            var name = String(method.token_net_name || '').toLowerCase();
            if (name === 'alipay') return 'stripe-alipay';
            if (name === 'wechat pay') return 'stripe-wechatpay';
            if (name === 'card') return 'stripe-card';
            return 'stripe-all';
        }
        return '';
    }

    function specialPaymentTypeFromOrder(order) {
        if (!order) return '';
        if (order.trade_type === 'alipay.mck') return 'alipay';
        if (order.trade_type === 'duolabao.qr') return 'duolabao';
        if (order.trade_type === 'stripe.alipay') return 'stripe-alipay';
        if (order.trade_type === 'stripe.wechatpay') return 'stripe-wechatpay';
        if (order.trade_type === 'stripe.card') return 'stripe-card';
        if (order.trade_type === 'stripe.all') return 'stripe-all';
        return '';
    }

    function specialPaymentLabel(type) {
        if (type === 'alipay') return t('alipayPaymentLabel', '支付宝付款');
        if (type === 'duolabao') return t('duolabaoPaymentLabel', '京东支付');
        if (type === 'stripe-alipay') return t('stripeAlipayPaymentLabel', '支付宝付款');
        if (type === 'stripe-wechatpay') return t('stripeWechatPayPaymentLabel', '微信支付');
        if (type === 'stripe-card') return t('stripeCardPaymentLabel', '银行卡支付');
        if (type === 'stripe-all') return t('stripePaymentLabel', 'Stripe 支付');
        return '';
    }

    function specialPaymentButtonI18nKey(type) {
        if (type === 'alipay') return 'openAlipayApp';
        if (type === 'duolabao') return 'openJdApp';
        if (type === 'stripe-alipay') return 'openStripeAlipay';
        if (type === 'stripe-wechatpay') return 'openStripeWechatPay';
        return '';
    }

    function specialPaymentButtonText(type) {
        var key = specialPaymentButtonI18nKey(type);
        if (key === 'openAlipayApp') return t(key, '打开 支付宝 APP');
        if (key === 'openJdApp') return t(key, '打开 京东 APP');
        if (key === 'openStripeAlipay') return t(key, '打开支付宝付款');
        if (key === 'openStripeWechatPay') return t(key, '打开微信支付');
        return '';
    }

    function specialPaymentTip(type) {
        if (type === 'alipay') {
            return t('alipayPaymentTip', '请使用国内版 支付宝 APP 付款，不支持海外支付宝/云闪付/美团等。<br>如发现转错，请勿尝试补齐差额，请联系商店工作人员全额退款。');
        }
        if (type === 'duolabao') {
            return t('duolabaoPaymentTip', '请使用京东 APP 付款，支持云闪付/微信支付。<br>部分客户微信支付可能因 IP 异常问题无法付款，请换用京东 APP 付款。');
        }
        if (type && type.indexOf('stripe-') === 0) {
            return t('stripePaymentTip', '您将跳转到 Stripe 官方结算页面完成付款。');
        }
        return '';
    }
    ...
    function updateAmount() {
        ...
        if (selMethod) {
            ...
            if (nEl) {
                var specialType = specialPaymentTypeFromMethod(selMethod);
                nEl.textContent = specialPaymentLabel(specialType) || (t('networkPrefix', '区块网络 · ') + selMethod.token_net_name);
            }
    ...
    function checkStatus() {
        ...
        fetch('/api/v1/pay/info', {
            ...
            body: JSON.stringify({ trade_id: tradeId, status_only: true })
        })
            .then(function (r) { return r.json(); })
            .then(function (res) {
    ...
    window.Payment = {
        ...
        specialPaymentTypeFromOrder: specialPaymentTypeFromOrder,
        specialPaymentLabel: specialPaymentLabel,
        specialPaymentButtonI18nKey: specialPaymentButtonI18nKey,
        specialPaymentButtonText: specialPaymentButtonText,
        specialPaymentTip: specialPaymentTip,
...
(function () {
    ...
    function configureAlipayAmountGate(d, paymentUrl) {
        var card = document.getElementById('alipayAmountConfirmCard');
        var amountEl = document.getElementById('alipayConfirmAmount');
        var fiatEl = document.getElementById('alipayConfirmFiat');
        var input = document.getElementById('alipayAmountInput');
        var copyBtn = document.getElementById('copyConfirmAmountBtn');
        var copyIcon = document.getElementById('copyConfirmAmountIcon');
        var qrWrapper = document.querySelector('#paymentStage .qr-wrapper');
        var nativePayButton = document.getElementById('nativePayButton');
        if (!card || !input) return;

        var requiredAmount = normalizePaymentAmount(d.actual_amount || d.money);
        var fiat = d.fiat || 'CNY';
        card.style.display = 'block';
        input.value = '';
        input.dataset.requiredAmount = requiredAmount;
        input.dataset.paymentUrl = paymentUrl || '';
        if (amountEl) amountEl.textContent = requiredAmount || d.actual_amount || d.money || '--';
        if (fiatEl) fiatEl.textContent = fiat;
        if (qrWrapper) qrWrapper.style.display = 'none';
        if (nativePayButton) nativePayButton.style.display = 'none';

        if (copyBtn && !copyBtn.dataset.bound) {
            copyBtn.dataset.bound = '1';
            copyBtn.addEventListener('click', function () {
                var amount = document.getElementById('alipayConfirmAmount');
                if (window.Payment && Payment.copyText) {
                    Payment.copyText(amount ? amount.textContent : '', _t('toastAmountCopied', '金额已复制'), copyIcon, true);
                }
            });
        }

        var revealPayment = function () {
            card.style.display = 'none';
            if (qrWrapper) qrWrapper.style.display = 'flex';
            if (nativePayButton) {
                nativePayButton.href = input.dataset.paymentUrl || '#';
                nativePayButton.style.display = 'flex';
                nativePayButton.classList.toggle('disabled', !input.dataset.paymentUrl);
            }
        };

        var showAmountError = function () {
            if (normalizePaymentAmount(input.value) !== input.dataset.requiredAmount) {
                if (window.Payment && Payment.showToast) {
                    Payment.showToast(_t('amountConfirmError', '输入错误，请再次确认金额后重新输入！'), 'error');
                }
                input.value = '';
                input.focus();
            }
        ...
        if (!input.dataset.bound) {
            ...
            input.addEventListener('input', function () {
                if (normalizePaymentAmount(input.value) === input.dataset.requiredAmount) revealPayment();
            });
            input.addEventListener('blur', function () {
                if (input.value) showAmountError();
            });
            input.addEventListener('keydown', function (event) {
                if (event.key === 'Enter' && input.value) showAmountError();
            });
        }
        input.focus();
    }

    function resetAlipayAmountGate() {
        var card = document.getElementById('alipayAmountConfirmCard');
        var input = document.getElementById('alipayAmountInput');
        var qrWrapper = document.querySelector('#paymentStage .qr-wrapper');
    ...
    function showPayment() {
        ...
        var paymentUrl = d.token || d.address || '';
        var specialType = Payment.specialPaymentTypeFromOrder ? Payment.specialPaymentTypeFromOrder(d) : '';
        var fiatAmount = specialType === 'alipay' ? (d.actual_amount || d.money) : d.money;
        window._qrAmount = amount;
        document.getElementById('orderMoneyQ').textContent = fiatAmount || '--';
        document.getElementById('orderFiatQ').textContent = d.fiat || '';
        document.getElementById('payAmountQ').textContent = amount + ' ' + currency;
        document.getElementById('payNetworkQ').textContent = (Payment.specialPaymentLabel ? Payment.specialPaymentLabel(specialType) : '') || (_t('networkPrefix', '区块网络 · ') + netName);
        document.getElementById('orderIdQ').textContent = d.order_id || '--';
        renderWalletAddress(paymentUrl);
        var addr = document.getElementById('addressLabelQ');
        if (addr) addr.textContent = _t('receivingAddress', '收款地址');
        $('#qrcode').empty().qrcode({ text: paymentUrl, width: 200, height: 200 });
        updateQrPaymentLogo(specialType ? '' : currency, network);
        var addressCard = document.getElementById('addressCard');
        var nativePayButton = document.getElementById('nativePayButton');
        var tipCard = document.getElementById('paymentTipCard');
        var tipText = document.getElementById('paymentTipText');
        if (specialType) {
            if (addressCard) addressCard.style.display = 'none';
            if (nativePayButton) {
                var buttonText = Payment.specialPaymentButtonText ? Payment.specialPaymentButtonText(specialType) : '';
                var buttonKey = Payment.specialPaymentButtonI18nKey ? Payment.specialPaymentButtonI18nKey(specialType) : '';
                nativePayButton.textContent = buttonText;
                nativePayButton.href = paymentUrl || '#';
                nativePayButton.style.display = buttonText ? 'flex' : 'none';
                nativePayButton.classList.toggle('disabled', !paymentUrl);
            ...
            if (tipText) tipText.innerHTML = Payment.specialPaymentTip ? Payment.specialPaymentTip(specialType) : '';
            if (specialType === 'alipay') configureAlipayAmountGate(d, paymentUrl);
            else resetAlipayAmountGate();
        } else {
            resetAlipayAmountGate();
            if (addressCard) addressCard.style.display = '';
            if (nativePayButton) {
                nativePayButton.style.display = 'none';
                nativePayButton.href = '#';
                nativePayButton.classList.remove('disabled');
                nativePayButton.removeAttribute('data-i18n');
            }
            if (tipCard) tipCard.style.display = 'none';
            if (tipText) tipText.innerHTML = '';
        }
