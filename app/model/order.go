...
type Order struct {
	...
	QrcodeURL         string     `gorm:"-" json:"qrcode_url,omitempty"` // 动态二维码链接地址，不写入数据库
...
func (o *Order) GetMethods(crypto Crypto) []MethodItem {
	...
	for tradeTypeStr, typeConf := range allTrades {
		...
		// 检查是否有可用钱包或通道
		var count int
		tradeType := TradeType(tradeTypeStr)
		if typeConf.TargetType == TargetTypeChannel {
			count = len(GetAvailableChannel(tradeType))
		} else {
			count = len(GetAvailableWallets(tradeType))
		}
		if count == 0 {
			continue
		}
