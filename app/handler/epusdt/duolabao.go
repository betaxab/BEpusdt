...
const duolabaoPaymentURLCachePrefix = "duolabao_payment_url:"
...
func cacheDuolabaoPaymentURL(order model.Order) {
	paymentURL := strings.TrimSpace(order.QrcodeURL)
	if paymentURL == "" || strings.TrimSpace(order.TradeId) == "" {
		return
	}
	ttl := time.Until(order.ExpiredAt)
	if ttl <= 0 {
		return
	}
	cache.Set(duolabaoPaymentURLCachePrefix+strings.TrimSpace(order.TradeId), paymentURL, ttl)
}

func cachedDuolabaoPaymentURL(tradeID string) (string, bool) {
	value, ok := cache.Get(duolabaoPaymentURLCachePrefix + strings.TrimSpace(tradeID))
	if !ok {
		return "", false
	}
	paymentURL, ok := value.(string)
	paymentURL = strings.TrimSpace(paymentURL)
	return paymentURL, ok && paymentURL != ""
}
