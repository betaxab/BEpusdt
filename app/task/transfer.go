			...
			for _, t := range batch {
				...
				if chOrders, ok := channelOrders[t.TradeType]; ok {
					...
					for _, o := range chOrders {
						// 金额前缀匹配
						if !amountMatch(t.Amount, o.Money, string(o.TradeType)) {
							continue
						}
						...
						var count int64
						model.Db.Model(&model.Order{}).Where("ref_orderno = ?", t.TxHash).Count(&count)
						if count > 0 {
						...
						// 找到匹配的订单
						o.MarkChannelConfirming(t.TxHash, t.FromAddress, t.Timestamp)
