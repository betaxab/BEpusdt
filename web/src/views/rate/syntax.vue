...
const showAtomModal = async () => {
  try {
    const res = await getsConfAPI({
      keys: ["atom_usdt", "atom_usdc", "atom_cnye", "atom_trx", "atom_eth", "atom_bnb", "atom_gram"]
    });
...
const handleAtomSubmit = async () => {
  try {
    if (!atomForm.usdt || !atomForm.usdc || !atomForm.cnye || !atomForm.trx || !atomForm.eth || !atomForm.bnb || !atomForm.gram) {
